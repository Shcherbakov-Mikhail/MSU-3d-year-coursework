...
INTEGER, PRIVATE   :: NSBD     ! КОЛИЧЕСТВО ОБРАЗЦОВ БД
INTEGER, PRIVATE   :: LSBD     ! НОМЕР      ОБРАЗЦА  БД

INTEGER, PRIVATE   :: KDBNN    ! НОМЕР КАНАЛА ВЫВОДА БД ANN
...
DO LSBD=1,NSBD ! ЦИКЛ ПО ВСЕМ ОБРАЗЦАМ БД

   ! ЗАДАНИЕ ПАРАМЕТРОВ ЖЕСТКОСТИ
   EMAT=DBMT(:,:,LSBD) ! МАССИВ ЖЕСТКОСТЕЙ КЭ
   ...
   RRQ=DBRQ(:,LSBD) ! МАССИВ ЖЕСТКОСТЕЙ КЭ
   ...
   ! ЗАПИСЬ ОБОБЩЕННЫХ ПЕРЕМЕЩЕНИЙ В БД ANN
   WRITE (KDBNN,*) PER

   ! ВИЗУАЛИЗАЦИЯ РЕЗУЛЬТАТОВ
   IF ( IP == 1 ) THEN ! ПАРАМЕТР УПРАВЛЕНИЯ ПОСТРОЕНИЕМ ЭПЮР
      CALL PBK1(LSBD)  ! НОМЕР РИСУНКА
   END IF

END DO

CLOSE (KDBNN) ! ЗАКРЫТИЕ ФАЙЛА БД ANN
...
INTEGER :: NVBD   ! КОЛИЧЕСТВО ПАРАМЕТРОВ В БД

INTEGER :: LSBD   ! СЧЕТЧИК ЦИКЛА: NSBD
INTEGER :: LMAT   ! СЧЕТЧИК ЦИКЛА: NMAT
INTEGER :: LRMAT  ! СЧЕТЧИК ЦИКЛА: NRMAT

CHARACTER (LEN=*), PARAMETER :: CHSUB ="RBD1BB"
CHARACTER (LEN=*), PARAMETER :: CHMSG1=CHMSG0//CHSUB//" МОДУЛЯ "//CHMDL
CHARACTER (LEN=*), PARAMETER :: CHERR1=CHERR0//CHSUB//" МОДУЛЯ "//CHMDL
...
READ (1,*) KDBNN, & ! НОМЕР КАНАЛА ВЫВОДА БД ANN
           KDBMT, & ! НОМЕР КАНАЛА  ВВОДА БД ЖЕСТКОСТЕЙ КЭ
...
IF ( ( KDBMT > 1 ) .OR. ( KDBMT < 100 ) ) THEN ! ВВОД БД ЖЕСТКОСТЕЙ КЭ
...
ELSE ! НОМЕР КАНАЛА ВВОДА БД ЖЕСТКОСТЕЙ КЭ !ELSE IF ( KDBMT /= 0 ) THEN
   ...
   " ОН ДОЛЖЕН БЫТЬ В ДИАПАЗОНЕ 2-99", CHERR3
   STOP

END IF

IF ( ( KDBRQ > 1 ) .OR. ( KDBRQ < 100 ) ) THEN ! ВВОД БД PACПPEДEЛEHHOЙ HAГPУЗKИ
   ...
   READ (KDBRQ,*) NSBD, & ! КОЛИЧЕСТВО ОБРАЗЦОВ БД
                  NVBD    ! КОЛИЧЕСТВО ПАРАМЕТРОВ В БД

   CALL AMFLT2 ("DBRQ", DBRQ, NVBD, NSBD, IRL=1) ! РАЗМЕЩЕНИЕ БД PACПPEДEЛEHHOЙ HAГPУЗKИ
...
OPEN(KDBNN, ACTION='WRITE', IOSTAT=IOSTAT) ! БД ANN

IF ( IOSTAT /= 0 ) THEN
   WRITE (3,'(/A/A/A,I0/A)') CHERR1, CHERR2,                           &
   " ПРОИЗОШЛА ОШИБКА ПРИ ОТКРЫТИИ ФАЙЛА БД ANN IOSTAT=", IOSTAT, CHERR3
   STOP
END IF

IF ( IW == 1 ) THEN
   WRITE (3,'(/A/A/A)') CHMSG1,              &
   " ВЫПОЛНЕН ВВОД БД ЖЕСТКОСТЕЙ КЭ",        &
   " ВЫПОЛНЕН ВВОД БД PACПPEДEЛEHHOЙ HAГPУЗKИ"
END IF

RETURN

END SUBROUTINE RBD1BB
